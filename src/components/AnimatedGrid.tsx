  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = runners.length - 1; i >= 0; i--) {
        ...
        // Draw Trail
        const size = 1; // 1px thickness matching grid
        r.trail.forEach((pos, index) => {
          const trailAlpha = (r.alpha * (1 - index / trailLength) * 0.5);
          if (trailAlpha > 0) {
            ctx.fillStyle = `rgba(255, 58, 2, ${trailAlpha})`; // Using primary color for trail
